...
import { AccountCategoryInfo } from '../types';
import { IconX, IconLoader } from './Icons';

interface AddAccountCategoryModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: Omit<AccountCategoryInfo, 'id'>) => Promise<void>;
  itemToEdit: AccountCategoryInfo | null;
  title: string;
}

const AddAccountCategoryModal: React.FC<AddAccountCategoryModalProps> = ({ isOpen, onClose, onSave, itemToEdit, title }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error("Failed to save account category:", error);
        } finally {
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md m-4" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="p-6 space-y-4">
                        <div>
                            <label htmlFor="name" className="block mb-2 text-sm font-medium text-slate-900">Category Name</label>
                    ...
                    <div className="flex items-center justify-end p-4 border-t bg-slate-50 rounded-b-lg">
                        ...
                        <button type="submit" disabled={isSubmitting} className="inline-flex items-center justify-center px-5 py-2.5 text-sm font-medium text-center text-white bg-brand-blue-600 rounded-lg hover:bg-brand-blue-700 focus:ring-4 focus:ring-brand-blue-300 disabled:bg-slate-400">
                            ...
                            {itemToEdit ? 'Save Changes' : 'Add Category'}
                        </button>
                    </div>
                </form>
            </div>
