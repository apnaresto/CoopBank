...
import { RM, Branch, BranchManager } from '../types';
import { IconX, IconLoader } from './Icons';

type ManagerData = Omit<RM, 'id'> | Omit<BranchManager, 'id'>;

interface AddManagerModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (data: ManagerData) => Promise<void>;
  title: string;
  branches: Branch[];
}

const AddManagerModal: React.FC<AddManagerModalProps> = ({ isOpen, onClose, onAdd, title, branches }) => {
    const initialState = { name: '', code: '', email: '', phone: '', branchId: '' };
    const [formData, setFormData] = useState<ManagerData>(initialState);
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg m-4" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="p-6 space-y-4">
                        ...
                        <div>
                            <label htmlFor="email" className="block mb-2 text-sm font-medium text-slate-900">Email Address</label>
                            <input type="email" name="email" id="email" value={formData.email} onChange={handleChange} required className="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-brand-blue-500 focus:border-brand-blue-500 block w-full p-2.5" />
                        </div>
                        <div>
                            <label htmlFor="phone" className="block mb-2 text-sm font-medium text-slate-900">Phone Number</label>
                            <input type="tel" name="phone" id="phone" value={formData.phone} onChange={handleChange} required className="bg-slate-50 border border-slate-300 text-slate-900 text-sm rounded-lg focus:ring-brand-blue-500 focus:border-brand-blue-500 block w-full p-2.5" />
                        </div>
                        <div>
